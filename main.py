...
sheet_id = "1eIoGfj35pMok5UAR8VH6BmCVzKY9trFH9S1RVwqJesI"
sheet = gc.open_by_key(sheet_id)

trooperSheet = sheet.worksheet("🚀TROOPER COMPANY")
...
async def AddUser(interaction: discord.Interaction, username: discord.Member):
    ...
    usernameCell = trooperSheet.find(userToAdd, in_column=2)
    ...
    for i, value in enumerate(trooperUsernameValue, start=20):
        ...
        memberDefaultData = [userToAdd, "Recruit", 0, 0, 0, 0, "FALSE", 0, f"=ARRAYFORMULA(IF(AND(E{i}>=3), TRUE, FALSE))", str(username.joined_at.date())]
...
async def GetUserInfo(interaction: discord.Interaction, user: discord.Member):
    ...
    if usernameCell.row == trooperSheet.row_count:
        ...
        trooperSheet.update(range_name=f"B{usernameCell.row}:K{usernameCell.row}", values=[["","","0","0","0","0","FALSE","0",f"=ARRAYFORMULA(IF(AND(E{usernameCell.row}>=3), TRUE, FALSE))",""]], value_input_option="USER_ENTERED")
    
    await interaction.response.send_message(content=f"{interaction.user.mention} User has been successfully removed from the database.")
...
async def CheckQuota(interaction: discord.Interaction):
    ...
    for i, (strikes, inactivity, quotaPassed, date, events) in enumerate(zip(quotaSrikes, hasInactivityNotice, hasCompletedQuota, joinDate, weeklyEvents), start=20):
        ...
        date = str(date[0]) if date else str(datetime.today().today)
        ...
        ########### MUST CHANGE ########### >> YYYY-MM-DD
        joinDateObj = datetime.strptime(date, "%d/%m/%y") ## Convert join date cell into datetime object to compare it against seven days ago
        ########### MUST CHANGE ########### >> YYYY-MM-DD
...
async def AddEventPoint(interaction: discord.Interaction, user: discord.Member):
    ...
    if not HasClearance(2, userToCheck):
        await interaction.response.send_message(ephemeral=True, content=f"{interaction.user.mention} You don't have permission to run this command.")
        return

    userToAddEP = user.nick or user.global_name or user.name
    usernameCell = trooperSheet.find(query=userToAddEP, in_column=2)

    if not usernameCell:
        await interaction.response.send_message(f"User {userToAddEP} wasn't found in DB")
        return
    
    rowToUpdate = trooperSheet.get(range_name=f"D{usernameCell.row}:F{usernameCell.row}")

    newValues = [[str(int(x) + 1) for x in sublist] for sublist in rowToUpdate]
    trooperSheet.update(range_name=f"D{usernameCell.row}:F{usernameCell.row}",values=newValues, value_input_option="USER_ENTERED")

    await interaction.response.send_message(f"DEBUGLOG: userToAddEP: {userToAddEP}. usernameCell: {usernameCell}, usernameCell.row: {usernameCell.row}. rowToUpdate: {rowToUpdate}")
